...
jQuery(document).ready(function ($) {
  ...
  function thisDiv(number, title) {
    const $items = $(".for-section-" + number);
    if ($items.length === 0) return;

    // Wrap content in card-body -> collapse -> div
    $items.wrapAll(`
      <div id="collapseSection${number}" 
           class="collapse order${number} wrap_div_${number}" 
           data-bs-parent="#accordion" 
           aria-labelledby="overall_div_${number}">
        <div class="card-body" id="card-section-${number}"></div>
      </div>
    `);
      ...
      <div id="overall_div_${number}" class="card-header ins-div wrap_div_${number}">
        <div class="row">
          ...
          <div class="col-md-6 text-end">
            <div id="section-${number}-button"
                 class="ins-col-test collapsed"
                 data-bs-target="#collapseSection${number}"
                 aria-expanded="false"
                 role="button"
                 aria-controls="collapseSection${number}">
    ...
    // Wrap everything into a card
    $(".wrap_div_" + number).wrapAll(
      `<div class="card ins-card order${number}"></div>`
    );
  }
