...
jQuery(document).ready(function ($) {
  ...
  $(".ins-col-test").on("click", function () {
    var $this = $(this);
    var $icon = $this.find("span");

    // Skip if it's an edit icon
    if ($icon.hasClass("fa-pen")) return;

    // Use setTimeout to let Bootstrap toggle `collapsed` class before checking it
    setTimeout(function () {
      if ($this.hasClass("collapsed")) {
        $icon.removeClass("fa-minus plus-minus").addClass("fa-plus");
      } else {
        $icon.removeClass("fa-plus").addClass("fa-minus plus-minus");
      }
    }, 10); // Short delay so Bootstrap's class toggle happens first
  });
